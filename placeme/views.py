...
def messages_api(request, session_id):
    ...
    if is_real_answer and session.current_question_count >= session.number_of_questions:
        ...
        try:
            ...
            closing_text = ""
            print(f"[messages_api] closing message error: {e}")

        session.is_active = False
        session.save()

        # Generate score and feedback — separate try/except so scoring errors don't hide the result
        from .utils import generate_score_and_feedback
        try:
            result = generate_score_and_feedback(session)
        except Exception as e:
            print(f"[messages_api] scoring error: {e}")
            result = {
                "score":      None,
