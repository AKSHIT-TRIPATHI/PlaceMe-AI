...
def _groq_generate(model: str, contents) -> str:
    ...
    [{role, text}] history dicts or types.Content objects.
    ...
    if isinstance(contents, str):
        ...
        for item in contents:
            if hasattr(item, "role") and hasattr(item, "parts"):
                # types.Content object from Gemini SDK
        ...
        # Groq requires strict user/assistant alternation starting with user.
        # Keep last 20 messages to avoid token overflow, always keep first (system prompt).
        if len(raw_messages) > 20:
            raw_messages = raw_messages[:1] + raw_messages[-19:]
        ...
        for msg in raw_messages:
            if msg["role"] == expected:
                ...
                # Skip out-of-order messages to maintain alternation
                continue

        # Must end with user role for the model to respond
    ...
    for model_name in GEMINI_MODELS:
        try:
            ...
            if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                ...
                time.sleep(1)
                continue
            # Non-quota ClientError — still try next model / Groq
            print(f"[utils] Gemini {model_name} ClientError: {e}, trying next…")
            last_exc = e
            continue
        except Exception as e:
